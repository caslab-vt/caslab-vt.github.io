...
# Project Overview
This project is a migration of a legacy 2018 academic laboratory website (CAS Lab) to a modern, static, Markdown-driven architecture using Astro 4.x and the `scholar-lite` theme. The goal is a highly maintainable, statically generated site.

# Tech Stack & Architecture
- **Framework:** Astro 4.x (Static Site Generation)
- **Styling:** Tailwind CSS (Strictly utility classes)
- **Content:** Markdown (`.md`) and MDX (`.mdx`) via Astro Content Collections
- **Data Validation:** Zod schemas
- **JavaScript:** Zero-JS by default. No jQuery.

# Core Development Rules

## 1. Content Collections & Data (The Source of Truth)
- All dynamic data (News, People, Publications, Research Areas) MUST be managed through `src/content/config.ts` using `defineCollection` and `zod` validation.
- Never hardcode list-based data (like lab news or alumni) directly into `.astro` UI files.
- Fail builds early: ensure Zod schemas strictly define types (e.g., `z.string()`, `z.date()`) for all frontmatter.

## 2. Component Engineering & Astro Best Practices
- Write concise, technical responses with accurate Astro examples.
- Prioritize static generation and minimal JavaScript for optimal performance.
- Use `.astro` files for all layouts and UI components.
- Implement strict TypeScript interfaces (`interface Props`) in the frontmatter of every component.
- Organize files using Astro's file-based routing system (`src/pages/`, `src/layouts/`, `src/components/`).
- Use Astro's built-in `<Image />` component (`import { Image } from 'astro:assets'`) for all local assets to ensure optimization.

## 3. Styling Constraints
- Use Tailwind utility classes extensively for responsive design (e.g., `sm:`, `md:`, `lg:`).
- **NEVER use the `@apply` directive.**
- Avoid custom `<style>` blocks in Astro components unless strictly necessary for scoped edge cases.

## 4. Execution & AI Behavior
- **Do not invent APIs or hallucinate data.** Rely strictly on the schemas defined in `src/content/config.ts`.
- When writing code, provide complete file outputs. Do not use placeholders like `// ... rest of code`.
- Ensure commit messages and terminal commands are ready to copy-paste.
